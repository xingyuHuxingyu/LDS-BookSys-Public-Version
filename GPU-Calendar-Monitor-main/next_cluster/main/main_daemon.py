...
class Cluster:
    ...
    GPU_WEIGHT = {
       ...
       # 定义 hostname 到显卡类型的映射字典
    HOSTNAME_TO_GPU = {
        'han5': 'NVIDIA GeForce RTX 2080 Ti',
        'sui1': 'NVIDIA GeForce RTX 3090',
        'sui2': 'NVIDIA GeForce RTX 3090',
        'sui3': 'NVIDIA GeForce RTX 3090',
        'sui4': 'NVIDIA GeForce RTX 3090',
        'sui5': 'NVIDIA GeForce RTX 3090',
        'tang1':'NVIDIA A40',
        'tang2':'NVIDIA A40',
        'tang3':'NVIDIA A40',
        'song1': 'NVIDIA A100-SXM4-80GB',
        'song2': 'NVIDIA A100-SXM4-80GB'
    }

      # 共用卡的资源消耗不能超过的最大算力单位
    MAX_RESOURCE_UNITS = 24  # 8张3090*3天
    ...
    def daemon_fetch_node(self, host_d: dict):
       ...
       ################################################### 
# 以下为新增或者调整部分：
    def get_gpu_weight(self, hostname):
        """
        根据hostname获取GPU的算力权重
        """
        gpu_type = self.HOSTNAME_TO_GPU.get(hostname, None)
        if gpu_type is not None:
            return self.GPU_WEIGHT.get(gpu_type, 1)  # 找到即返回对应权重，找不到默认为1
        return 1  # 默认权重，假设权重为1
    ...
    def add_booking_check(self, df: pd.DataFrame):
        ...
        # Merge previous violations to original dataframe
        if len(violate_ngpu) > 0:
        ...
        #################新增GPU资源消耗计算#################
        # 计算每个用户的GPU资源消耗(算力权重)
        df['day_count'] = df.groupby(['title', 'hostname'])['day'].transform('count')

        # 获取 GPU 权重，并计算每个用户的资源消耗
        df['resource_units'] = df['hostname'].apply(self.get_gpu_weight)

        # 根据 title 分组，并计算每个用户的总资源消耗
        group = df.groupby('title')
        u2resource = group['resource_units'].sum()

        # 检查用户是否超过资源限额
        violate_resource = u2resource.apply(lambda k: k > self.MAX_RESOURCE_UNITS)



        # 如果检测到 violate_resource 违约，将对应的 violate_ngpu 也设置为违约
        violate_titles = violate_resource[violate_resource].index.tolist()
        df.loc[df['title'].isin(violate_titles), 'violate_ngpu'] = True

        #######################GPU算力换算#############

        # invalid book, violate max gpu, violate max day, violate resource
        # 合并所有违约情况，确保任何一种违约都能被检测出来
        def status2code(inv_bk, vio_gpu, vio_day):
            args = [inv_bk, vio_gpu, vio_day]
            if any(args):
                return args.index(True) + 1  # 第一个出现True的违约
            else:
                return 0  # 无违约
    ...
    def _psudo_node(self, host):
        ...
        if len(all_ind) > 0:
            n = all_ind.max() + 1 # 修改该错误，int型无item()属性。该bug导致无法更新 n = all_ind.max().item() + 1
        else:
            n = 0
        gpus = [{'index': i,
                 'name': '',
...
if __name__ == '__main__':
    ...
    host_data = [ 
        {"nickname": "han5", "ip": "192.168.1.6"},
        {"nickname": "sui1", "ip": "192.168.1.10"},
        {"nickname": "sui2", "ip": "192.168.1.9"},
        {"nickname": "sui3", "ip": "192.168.1.8"},
        {"nickname": "sui4", "ip": "192.168.1.5"},
        {"nickname": "sui5", "ip": "192.168.1.7"},
        {"nickname": "tang1", "ip": "192.168.1.4"},
        {"nickname": "tang2", "ip": "192.168.1.12"},
        {"nickname": "tang3", "ip": "192.168.1.13"},
        {"nickname":"song1", "ip": "192.168.1.11"},
        {"nickname":"song2","ip":"192.168.1.50"}
    ]
    add_cld = True#len(args.teamup_ids) > 0
