...
def get_gpu_serial() -> Dict[str, int]:
    ...
    for gpu_idx in range(N.nvmlDeviceGetCount()):
        handle = N.nvmlDeviceGetHandleByIndex(gpu_idx)
        # try:
        #     serial = N.nvmlDeviceGetSerial(handle).decode()
        # except N.NVMLError_NotSupported:
        #     print(f"Serial not supported for GPU {gpu_idx}, using UUID instead.")
        serial = N.nvmlDeviceGetUUID(handle)
        if isinstance(serial, bytes):
            serial = serial.decode()
        ser_map[serial] = gpu_idx
    N.nvmlShutdown()
    return ser_map
...
def get_gpu_process(serial_map: Optional[Dict[str, int]])->Dict[int, List[Dict[str, Any]]]:
    ...
    df['idx'] = df['gpu_uuid'].apply(lambda k: serial_map[k])
